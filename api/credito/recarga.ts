...
export async function POST(req: Request): Promise<Response> {
  ...
  const valor = Number(body.valor_brl);
  if (!valor || valor <= 0) return json({ error: "valor_brl deve ser um número > 0." }, 400);
  try {
    const r = await fetch(base.replace(/\/+$/, "") + "/rest/v1/credito_recargas", {
      method: "POST",
      headers: { apikey: key, Authorization: "Bearer " + key, "Content-Type": "application/json", Prefer: "return=minimal" },
      body: JSON.stringify({ valor_brl: valor, descricao: String(body.descricao || "").slice(0, 200) || null }),
    });
    if (!r.ok) {
      const txt = (await r.text().catch(() => "")).slice(0, 200);
      return json({ error: `Falha ao registrar (${r.status}): ${txt}` }, 502);
    }
    return json({ ok: true });
  } catch (e: any) {
